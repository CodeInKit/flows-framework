...
async function registerFlows(data, unsafe) {
  await Promise.each(fs.readdir(data.normalizedFlowsPath), async (filename) => {
    ...
    const isNamespace = await checkNamespace(data.normalizedFlowsPath + '/' + filename + '/index.js', stats).catch(() => true);
    ...
    namespaceFlows.forEach((nfilename) => {
      ...
      debug(`register ${!_.isEmpty(nfilename) ? filename + '/' : ''}${filenameWithoutSuffix} with actions [${_.map(flow, f=> f.name).join(', ')}]`)
      unsafe.flows.register(`${!_.isEmpty(nfilename) ? filename + '/' : ''}${filenameWithoutSuffix}`, flow);
    });
  });
...
module.exports = {
  ...
  async init(dirname, config = {}) {
    ...
    initFlows.hook('exception', e => console.log(JSON.stringify(e)))
